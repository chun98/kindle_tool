...
class Application(tk.Frame):
    def __init__(self, master=None):
        ...
        self.pack()
        self.create_widgets()
        master.geometry("400x150+100+100")
        master.resizable(False,False)
        master.title("Kindle Notes")


    def create_widgets(self):
        self.hi_there = tk.Label(self)
        self.hi_there["text"] = "Kindle Notes可以将MyClippings.txt导出为HTML文件"
        self.hi_there.pack(side="top")
        self.path_entry = tk.Entry(self)
        self.path = tk.StringVar()
        self.path_entry["textvariable"]=self.path
        self.path_entry.pack(fill='both')
        self.select_button=tk.Button(self,command=self.select)
        self.select_button["text"]="选择路径"
        self.select_button.pack(side="right")
        self.translate = tk.Button(self,command=self.translator)
        self.translate["text"]="开始转换"
        self.translate.pack(side="left")

    def select(self):
        path_choice=filedialog.askopenfilename()
        self.path.set(path_choice)
    ...
    def translator(self):
        ...
        if (os.path.exists(path)):
                try:
                    path=path.split("/")[-1]
                    if(path=="My Clippings.txt"):
                        run(path)
                    else:
                        messagebox.showwarning("提示","错误文件")
                except:
                    messagebox.showwarning("提示","转换失败")
        else:
